...
`/test/browser/selection-summary.html` は、ストリームイベントを模擬して選択要約の初回と長さ調整の途中表示・失敗時の結果復元と再試行・追加指示の会話（履歴・タイトル）・閉じた後の遅延応答を確認します。成功時のタイトルは `PASS 選択要約テスト` です。実モデルの要約品質は、拡張と対象ページを再読み込みし、文章を選択 →「LLM要約」→「短く」「長く」、追加指示で確認してください。

追加指示と長さ調整の相互排他、最新回答からの調整、フォーム・履歴の保持、開始応答より先に完了・失敗した場合の操作復帰も同ページで検証します。
