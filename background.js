...
log.info('background', 'Service Worker 起動');
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  log.info('background', 'メインの background.js がメッセージを受信', message);
  // handleBackgroundMessage内で非同期処理が行われる可能性を示唆するために true を返す
  const result = handleBackgroundMessage(message, sender, sendResponse);
  log.info('background', 'handleBackgroundMessage の戻り値', { result });
  return result;
});


log.info('background', 'background.js の初期化完了');
